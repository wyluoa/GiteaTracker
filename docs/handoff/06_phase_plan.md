...
## Phase 2 — Cell 編輯 + Timeline + 會議紀錄(✅ 已完成)
...
## Phase 5 — Dashboard + Calendar + Closed + 批次操作(✅ 已完成)
...
- `.drone.yml`:checkout、install deps、smoke test、SSH deploy(使用 deploy scripts,不需 sudo)
- nohup + PID file 管理(不使用 systemd,無 sudo 權限)
- deploy/ 目錄:start.sh、stop.sh、status.sh、backup.sh、restore.sh
- 部署 / 升級流程文件(`deploy/DEPLOY.md`,含 bash + csh 指令對照)
- 備份 / 還原流程(含 user crontab 自動每日備份)

### 6.5 Excel 上傳更新(✅ 已完成)

- Admin → Excel Update:super user 可透過網頁上傳新 Excel 更新資料
- 差異預覽:上傳後顯示新增/修改的 issue 及欄位差異
- 衝突標示:DB 與 Excel 都有不同值時黃底標示「conflict」
- 逐欄勾選:使用者可選擇哪些變更要套用
- Timeline + audit log:所有變更記錄在 timeline 及 audit log
...
1. Day 1:用 CLI `import_from_excel.py` 一次匯入新系統作初始資料
2. 並行期間:Excel 仍是 single source of truth,新系統用來驗證功能
3. 穩定後:最後一次匯入覆蓋,停用 Excel,正式切換
4. 切換後:如需用 Excel 更新資料,可透過 Admin → Excel Update 網頁上傳,系統會顯示差異預覽、標示衝突,確認後才寫入
